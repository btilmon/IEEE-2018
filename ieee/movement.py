...
############################################################
        ...
        if case == 1:
            ...
            right_thres = np.array([850, 500, 600])
            
            left_dist = distance_slice(x, 270, 15)
            right_dist = distance_slice(x, 90, 15)
            
            #~ right.Drive(150, 0)
            #~ left.Drive(200, 1)
            
            #~ while np.min(left_dist) < left_thres[orbit_group]:
                #~ right.Drive(200, 0)
                #~ left.Drive(150, 1)
                #~ x = read()
                #~ left_dist = distance_slice(x, 290, 15)
                #~ right_dist = distance_slice(x, 90, 15)    
