...
try:
    ...
    d_thres = np.arange(scan.distance-15, scan.distance+15)
    print(d_thres)

    zero = np.array([])
    zero = np.append(scan.x[0:5,0],zero)
    zero = np.append(scan.x[scan.x.shape[0]-5:scan.x.shape[0],0],zero)

    while(np.isin(zero.any(),d_thres.any())==True):
        lidar = scan.lidar
        x = lidar.Scan(scan.reads)
        zero = np.array([])
        zero = np.append(scan.x[0:5,0],zero)
        zero = np.append(scan.x[scan.x.shape[0]-5:scan.x.shape[0],0],zero)
    ##    d_thres = np.arange(scan.distance-15, scan.distance+15)
